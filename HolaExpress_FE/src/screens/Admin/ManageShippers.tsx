...
export default function ManageShippers({ navigation }: any) {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [shippers, setShippers] = useState<UserSummary[]>([]);
  const [filteredShippers, setFilteredShippers] = useState<UserSummary[]>([]);
  ...
  useEffect(() => {
    filterShippers();
  }, [shippers, searchQuery, selectedFilter]);

  const loadShippers = async () => {
    try {
      setLoading(true);
      const response = await adminService.getUsers(currentPage, 20, 'SHIPPER');
      
      if (response.success && response.data) {
        const shippersData = response.data.users || [];
        setShippers(shippersData);
        setTotalPages(response.data.totalPages || 1);
        calculateStats(shippersData);
      } else {
        Alert.alert('Lỗi', response.message || 'Không thể tải danh sách Shipper');
      }
    } catch (error) {
  ...
  const filterShippers = () => {
    ...
    if (selectedFilter !== 'all') {
      filtered = filtered.filter(s =>
        selectedFilter === 'active' ? s.isActive : !s.isActive
      );
    }
  ...
  const handleToggleStatus = async (userId: number, currentStatus: boolean) => {
    try {
      ...
      Alert.alert(
        'Xác nhận',
        `Bạn có chắc muốn ${action} tài khoản Shipper này?`,
        [
  ...
  const renderShipperCard = (shipper: UserSummary) => (
    <View key={shipper.userId} style={styles.shipperCard}>
      <View style={styles.shipperHeader}>
        <View style={styles.shipperImageContainer}>
          {shipper.avatarUrl ? (
            <Image source={{ uri: shipper.avatarUrl }} style={styles.shipperImage} />
          ) : (
            <View style={styles.shipperImagePlaceholder}>
              <MaterialCommunityIcons name="bike" size={30} color="#999" />
            </View>
          )}
        </View>
        ...
        <View style={styles.shipperActions}>
          <TouchableOpacity
            style={[styles.statusBadge, { backgroundColor: shipper.isActive ? '#4CAF50' : '#F44336' }]}
            onPress={() => handleToggleStatus(shipper.userId, shipper.isActive)}
          >
            <Text style={styles.statusText}>
              {shipper.isActive ? 'Hoạt động' : 'Khóa'}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
      <View style={styles.shipperFooter}>
        <Text style={styles.shipperDate}>
          Tham gia: {new Date(shipper.createdAt).toLocaleDateString('vi-VN')}
        </Text>
      </View>
    </View>
  );
    ...
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={['#FF6B35', '#F7931E']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <TouchableOpacity onPress={loadShippers} style={styles.refreshButton}>
      ...
      >
        ...
        <View style={styles.statsContainer}>
          {renderStatCard('Tổng Shipper', stats.total, 'bike', '#2196F3')}
        ...
        <View style={styles.listContainer}>
          {filteredShippers.length > 0 ? (
            filteredShippers.map(renderShipperCard)
          ) : (
            <View style={styles.emptyContainer}>
              <MaterialCommunityIcons name="bike-fast" size={60} color="#CCC" />
              <Text style={styles.emptyText}>Không tìm thấy Shipper nào</Text>
            </View>
          )}
        </View>
